...
from gsm8k_rope import FillerTester, debug, info
import gsm8k_rope
...
class ExperimentSpec:
    ...
    def from_dict(spec_obj, name=None):
        ...
        if isinstance(spec_obj, dict):
            return ExperimentSpec(name, spec_obj.get('type'), spec_obj.get('range'), spec_obj.get('choices'))
...
class Experiment:
    def __init__(self, name, version, experiment_type, linking_tester_class,
                 the_results_directory=None, naming_style=None):
        """
        ...
        self.linking_tester_class = globals()[linking_tester_class] if isinstance(linking_tester_class, str) else linking_tester_class
    ...
    def register_test_directory(self, directory):
        ...
        if specs_dict is None:
            specs_dict = self.linking_tester_class.parse_conventional_name(directory, self.experiment_name_for_tester_legacy())
        if specs_dict is None:
            debug(f"  Cannot get specs from naming. Looking for {directory}/test_info.json ...")
            if os.path.exists(os.path.join(directory, "test_info.json")):
                with open(os.path.join(directory, "test_info.json"), "r") as f:
                    ...
                    if not specs_dict:
                        debug(f"  {directory}/test_info.json found but there is not specs. Ignoring this directory.")
                        return False
            else:
                debug(f"  {directory}/test_info.json does not exist. Ignoring this directory.")
                return False
        debug(f"  specs extracted: {TestSpecs.from_pure_dict(specs_dict).to_string()}")
        self.test_directories[TestSpecs.from_pure_dict(specs_dict).to_string()] = {"path": directory, "status": "not open"}
        debug(f"  Registered. ")
    ...
    def find_existing_tests(self, force_update=True):
        ...
        if self.naming_style == "legacy" or self.naming_style is None:
            template_legacy = self.linking_tester_class.conventional_naming_template(self.experiment_name_for_tester_legacy())
    ...
    def get_experiment_specs(self):
        default_specs = self.type_config.get("specs", {})
    ...
    def run_tester(self, tester, callback_function=None, stop_event=None, **dynamic_running_specs):
        ...
        if isinstance(dynamic_running_specs.get("dynamic_running_specs"), ExperimentSpecs):
            test_specs = TestSpecs(dynamic_running_specs.get("dynamic_running_specs").specs)
        else:
        ...
        tester.standard_run_test_api(callback_function=callback_function, stop_event=stop_event, **running_specs.to_dict())
    ...
    def run_all_tests(self, stop_event=None, **dynamic_running_specs):
        ...
        def refresh_table(move_cursor=False):
            nonlocal table, mws
            if gsm8k_rope.output_level != "debug" and move_cursor:
        ...
        for i, specs in enumerate(all_specs):
            ...
            if not things or "tester" not in things:
                ...
                refresh_table(True)
                yield augmented_all_specs
                tester = self.create_tester_with_test_specs(specs, savefile_root=root_directory,
...
class ExperimentManager:
    ...
    def __init__(self, the_results_directory=None):
    ...
    def get_slurm_jobs(self):
        squeue_results = self.get_bs().get_slurm_jobs().filter(USER=os.environ["USER"])
        squeue_results = squeue_results.get_columns(["JOBID", "ST", "NODES", "NODELIST(REASON)"])
        nodes_cnt = 0
        for nodes in squeue_results["NODES"]:
            nodes_cnt += int(nodes)
        with self.resource_lock:
            return {"headers": squeue_results.headers, "data": squeue_results.data, "nodes_cnt": nodes_cnt,
                    "auto_submit": self.bs_config.get("auto_submit", {"mode": "off"})}

    def get_slurm_settings(self):
        return self.bs_config.get("slurm_default_kwargs", "No settings. You have to provide one!\n"
    ...
    def manage_auto_submit_job(self, mode="on", nodes="8"):
        with self.resource_lock:
            self.bs_config["auto_submit"] = {"mode": mode, "nodes": nodes}
        def auto_submit_job_worker():
            while True:
                with self.resource_lock:
                    auto_submit_settings = self.bs_config.get("auto_submit", {"mode": "off", "nodes": "0"})
    ...
    def import_slurm_settings_from_file(self, filename):
        ...
        self.bs_config["slurm_default_kwargs"] = kwargs
        return kwargs

    def submit_slurm_job(self, **kwargs):
        sbatch_default_kwargs = self.bs_config.get("slurm_default_kwargs", {})
    ...
    def open_experiment(self, experiment_name=None, experiment_version=None, experiment_type=None,
        ...
        if temp_experiment:
            if temp_experiment.directory_full_path() in self.list_of_experiments and ignore_if_open:
                print(f"Experiment already open: {self.list_of_experiments[temp_experiment.directory_full_path()].test_directories}")
    ...
    def run_one_experiment(self, experiment, interactive=False, update_node_list=True,
        ...
        for interactive_result in experiment.run_all_tests(stop_event=stop_event,
                                                           url_list=",".join(self.bs.node_list_with_ports), **kwargs):
            if interactive:
                yield {
